...
if mod==1
    if factor==1
        Simceliang1=(Freference1_obj.*exp(2*1i*x(5)*10^(-2)*2*pi*ff1/cc)).*Hw;
    else
        Simceliang1=(Freference1_obj.*exp(2*1i*x(7)*10^(-2)*2*pi*ff1/cc)).*Hw;
    end
else
    if factor==1
        Simceliang1=(Freference1_obj.*exp(2*1i*x(6)*10^(-2)*2*pi*ff1/cc)).*Hw;
    else
        Simceliang1=(Freference1_obj.*exp(2*1i*x(9)*10^(-2)*2*pi*ff1/cc)).*Hw;
    end
end
for i=1:m1
    Simceliang2(i)=real(Simceliang1(m1-i+1))-1i*imag(Simceliang1(m1-i+1));
end
Simceliang=[zeros(1,ml1-1),Simceliang1,zeros(1,ml3-ml2-1),Simceliang2,...
    zeros(1,N-ml4)];
TDsimceliang=ifft(Simceliang);
y=sum((measure1-TDsimceliang).^2);
end